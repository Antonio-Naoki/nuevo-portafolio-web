...
export default function Projects() {
  ...
  return (
    <section
      id="projects"
      ref={ref}
      className="min-h-screen py-12 xs:py-14 sm:py-16 md:py-20 px-4 sm:px-6 md:px-8 bg-white dark:bg-transparent subtle-texture"
    >
      <motion.div style={{ opacity, y }} className="max-w-6xl mx-auto">
        ...
        {projects
          ...
          .map((project) => (
            ...
            >
              <div className="bg-gray-100/80 dark:bg-gray-900/50 backdrop-blur-sm rounded-xl overflow-hidden border border-gray-300 dark:border-gray-800 gradient-border card-hover">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 xs:gap-4 sm:gap-5 md:gap-6">
                  <div className="relative h-48 xs:h-52 sm:h-56 md:h-auto order-1 overflow-hidden group">
                    ...
                    <div className="absolute top-3 sm:top-4 left-3 sm:left-4 z-10">
                      <Badge className="bg-gradient-to-r from-blue-600 to-purple-600 text-white text-[10px] xs:text-xs shadow-md">
                        Destacado
                      </Badge>
                    </div>
                  </div>
                  <div className="p-3 xs:p-4 sm:p-5 md:p-6 flex flex-col justify-center order-2">
                    ...
                    <div className="flex space-x-3 xs:space-x-4">
                      <motion.a
                        href={project.github}
                        className="text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors flex items-center gap-1.5 xs:gap-2 group"
                        target="_blank"
                      ...
                      >
                        <Github
                          size={14}
                          className="xs:w-4 xs:h-4 sm:w-5 sm:h-5 group-hover:rotate-12 transition-transform"
                        />
...
function ProjectCard({ project, index }: { project: (typeof projects)[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="bg-gray-100/80 dark:bg-gray-900/50 backdrop-blur-sm rounded-xl overflow-hidden border border-gray-300 dark:border-gray-800 gradient-border card-hover"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <div className="relative h-32 xs:h-36 sm:h-40 md:h-48 overflow-hidden group">
        <motion.div
          style={{
            y: isHovered ? -10 : 0,
        ...
        <div className="absolute top-3 left-3 z-10">
          <Badge
            variant="outline"
            className="bg-purple-900/20 text-purple-400 border-purple-500/30 text-[10px] xs:text-xs backdrop-blur-sm"
          >
            {project.type}
          </Badge>
        </div>
      ...
      <div className="p-3 xs:p-4 sm:p-5 md:p-6">
        ...
        <div className="flex space-x-3 xs:space-x-4">
          <motion.a
            ...
            transition={{ type: "spring", stiffness: 400, damping: 10 }}
          >
            <Github size={14} className="xs:w-4 xs:h-4 sm:w-5 sm:h-5 group-hover:rotate-12 transition-transform" />
